...
<div class="container-fluid">
    <form id="dtr-form" action="/DTR/{{$employee->id}}" method="get">
        ...
        <div class="form-group col-sm-4 m-0">
            ...
            <div class="row">
                <span class="m-2">{{$employee->firstname}} {{$employee->middlename}} {{$employee->lastname}}</span>
                {{-- <dtr range="{{$date_range}}" user_id="{{$employee->id}}"></dtr> --}}
                {{-- <a href="{{url("/DTR/$date_range/Employee/$user_id/Process")}}" class="btn btn-primary text-light"
                data-toggle="tooltip" data-placement="bottom" title="Process All"><i class="fa fa-repeat"></i></a> --}}
            </div>
        </div>
    </form>
        ...
        <table class="table table-inverse">
            <thead class="thead-inverse thead-sticky">
                ...
                <tr>
                    ...
                    <th rowspan="2" class="bg-dark text-light align-middle">Leave</th>
                    <th rowspan="2" class="bg-dark text-light text-center align-middle">Total <small
                            class="d-block text-center">Hrs. Rendered</small></th>
                </tr>
                <tr>
                    <th class="bg-green-300 text-center align-middle text-xs">In</th>
                    <th class="bg-green-300 text-center text-xs" data-toggle="tooltip" data-placement="bottom"
                        title="Break-Out">BO</th>
                    <th class="bg-green-300 text-center text-xs" data-toggle="tooltip" data-placement="bottom"
                        title="Break-In">BI</th>
                    <th class="bg-green-300 text-center text-xs align-middle">Out</th>
                    <th class="bg-blue-300 text-center align-middle text-xs">In</th>
                    <th class="bg-blue-300 text-center text-xs" data-toggle="tooltip" data-placement="bottom"
                        title="Break-Out">BO</th>
                    <th class="bg-blue-300 text-center text-xs" data-toggle="tooltip" data-placement="bottom"
                        title="Break-In">BI</th>
                    ...
                    <th class="bg-red-300 align-middle text-center text-xs" data-toggle="tooltip"
                        data-placement="bottom" title="Undertime">UT</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($dtr_view as $record)
                    ...
                    @elseif (in_array("paid VL", json_decode($record['tag'])))
                        <tr class="text-xs bg-purple-400">
                    @elseif (in_array("VL", json_decode($record['tag'])))
                        <tr class="text-xs bg-purple-500 text-light">
